...
def download_latest_samples(destination):
    ...
    for source in [
            'kxp174430@ganymede:~/*_powerlaw_alllock_mcmc_samples.h5',
...
def plot_single_constraint(samples, system_name, axis, config):
    """Plot the constraint for a single system."""

    print('axis: ' + repr(axis))
    orig_axis = pyplot.gca()
    pyplot.sca(axis)
    frequency_dependence_plotter = PowerlawLgQDependencePlotter(1, config)
    frequency_dependence_plotter.add_chain(samples,

...
class W19EccentricityDistribution(stats.rv_continuous):
    """KDE estimate of the eccentricity distribution from W19 samples."""
    ...
    def _get_eccentricity_kernel_width(self, kic_id):
        """Return dict of eccentricity kernel widths for each KIC."""

        return self._manual_kernel_widths.get(kic_id, (1e-4, 1e-5))


    def _find_support(self, eccentricity_pdf, median_eccentricity):
        """Return the range outside of which the PDF(e) can be neglected."""

        def to_solve(eccentricity):
            pdf = eccentricity_pdf(eccentricity)
        ...
        self._logger.debug('Approximate support: %s < e < %s',
                      repr(min_e),
                      repr(max_e))
    ...
    def _interpolate(self,
        ...
        approximation_config['ext'] = 1

        pickle_file = MultiPickle(pickle_fname, ignore_config_args=['ext'])
        ...
        if pickled is not None:
            ...
            self._logger.debug('Unpickled: %s', repr(pickled))
            self._pdf_interp = InterpolatedUnivariateSpline(
                *pickled,
                **approximation_config
            )
        else:
            w19_samples = get_samples(kic_id)
            e_samples = numpy.sqrt(w19_samples['esinw']**2
                                   +
                                   w19_samples['ecosw']**2)
            ...
            (
                sin_kernel_width,
                cos_kernel_width
            ) = self._get_eccentricity_kernel_width(kic_id)
            ...
            x_values, y_values = approximate_1d_function(
                kde_distro.pdf,
                self._find_support(kde_distro.pdf, numpy.median(e_samples)),
                min_grid_step=1e-8,
                grid_only=True,
                **approximation_config
            )
        ...
        self._cdf_interp = self._pdf_interp.antiderivative()


    #pylint: disable=arguments-renamed
    #pylint: disable=arguments-differ
    def _pdf(self, eccentricity):
        """Return the PDF(e) for the given eccentricity."""

        return self._pdf_interp(eccentricity) / self._norm


    def _cdf(self, eccentricity):
        """Return the CDF(e) for the given eccentricity."""

        return self._cdf_interp(eccentricity) / self._norm
    ...
    def __init__(self,
                 ...
                 pickle_lock=None,
                 **kwargs):
        """Set up the distribution using the samples for the given KIC."""

        self._interpolate(kic, pickle_fname, pickle_lock)
        support = self._pdf_interp.get_knots()[[0, -1]]
        self._norm = self._cdf_interp(support[1])
        super().__init__(a=support[0],
                         b=support[1],
                         name=f'{kic!s}_eccentricity',
                         **kwargs)
...
def evaluate_eccentricity_pdf(kic_id, pickle_lock=None):
    ...
    e_distro = W19EccentricityDistribution(kic_id, pickle_lock=pickle_lock)
    plot_x = [numpy.linspace(*e_range, 1000) for  e_range in plot_ranges]
    plot_y = e_distro.pdf(plot_x)

    return  plot_ranges, plot_x, plot_y, e_distro.support()
...
def plot_eccentricity_distribution(kic_id_list,
    ...
    if num_parallel_processes > 1:
        with Manager() as pool_manager:
            pickle_lock = pool_manager.Lock()
            with Pool(num_parallel_processes) as pool:
                kde_plot_data = pool.map(
                    partial(evaluate_eccentricity_pdf, pickle_lock=pickle_lock),
                    kic_id_list
                )
    else:
        kde_plot_data = [evaluate_eccentricity_pdf(kic) for kic in kic_id_list]
    ...
    for kic_id, kde_plot_data in zip(kic_id_list, kde_plot_data):
        ...
        tight = False
        support = kde_plot_data[-1]
        for e_range, plot_x, plot_y in zip(*kde_plot_data[:-1]):
            ...
            pyplot.axvline(support[0], color='green')
            pyplot.axvline(support[1], color='green')

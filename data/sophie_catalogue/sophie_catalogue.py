"""Implement a class for working with the SOPHIE catalogue of binaries."""
import re
import os.path
...
class SophieCatalogue:
    """Interface to the data in Kiefer et. al. 2019."""

    #Due to copy-pasted data.
    #pylint: disable=too-many-statements
    @staticmethod
    def _get_supplementary_targets():
        ...
        def parse_string_with_errors(column_str,
                                     values,
                                     plus_errors,
                                     minus_errors):
            """
            ...
            Args:
                values(1-D array):    A pre-allocated array to fill with the
                    parsed values.

                plus_errors(1-D array):    A pre-allocated array to fill with
                    the parsed errors toward larger values.

                minus_errors(1-D array):    A pre-allocated array to fill with
                    the parsed errors toward smaller values.
        ...
        def parse_string_list(column, values, plus_errors, minus_errors):
            """Parse a column stored as a list of one strings for each entry."""
...
#        result['Dec'] = parse_coordinate(raw_data['dec'])
    ...
    @staticmethod
    def _read_tsv_file(fname, names_row):
    ...
    def _parse_target_info(self):
        """Return a structured array combining tables B1 and B3."""

        return pandas.merge(self._read_tsv_file('table_B1', 62),
                            self._read_tsv_file('table_B3', 60),
                            how='outer',
                            on='Name',
                            validate='1:1')

    def _parse_orbits(self):
        return pandas.concat([
            self._read_tsv_file('table_B5', 77),
            self._read_tsv_file('table_B6', 77),
        ])

    def __init__(self):
        """Prepare the catalogue data for use."""

        self._primary_targets = pandas.merge(
        ...
        self._supplementary_targets = pandas.DataFrame(
            self._get_supplementary_targets()
        )
        print('Primary targets:')
        print('Keys: ' + repr(self._primary_targets.keys()))
        print(self._primary_targets)
        print('Supplementary targets:')
        print('Keys: ' + repr(self._supplementary_targets.keys()))
        print(self._supplementary_targets)
    ...
    def __call__(self, **identifier):
        ...
        Args:
            identifier:    Should be either `HD=<HD ID>` if an HD number is
                avalable, or `BD=<BD ID>` otherwise, as these are the only
                identifiers in the tables.

        Returns:
            pandas dataframe:
                All available information on the system with the given
                identifier in the SOPHIE catalogue.
        """

        id_str = ''.join(next(iter(identifier.items())))
        print('Checking SOPHIE for ' + id_str)
        primary_data = self._primary_targets[self._primary_targets['Name']
                                             ==
                                             ('%-9.9s' % id_str)]
        if primary_data.empty:
            supplementary_data = self._supplementary_targets[
                self._supplementary_targets['name'] == id_str.encode('ascii')
            ]
            if supplementary_data.empty:
                print('Not found')
                return None
            print('Supplementary')
...
if __name__ == '__main__':
    catalogue = SophieCatalogue()
    print('Info for: ' + 'HD85533')
    print(catalogue(HD='85533'))

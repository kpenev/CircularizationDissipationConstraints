...
class EccentricitySolverCallable:
    ...
    def __call__(self, initial_eccentricity):
        ...
        final_eccentricity = self.system.pl_orbeccen[i] #find_ic.binary.final_state().eccentricity
        #pylint: enable=no-member
        ...
        return final_eccentricity #- self.system.pl_orbeccen[i]
    ...
    def get_found_evolution(self,
        ...
        binary = self._create_system(
            ...
            #False positive
            #pylint: disable=no-member
            porb_initial=self.porb_initial, #.to(units.Gyr).value,
            #pylint: enable=no-member
            initial_eccentricity=initial_eccentricity
        )
        binary.evolve(
...
def find_evolution(system,
    ...
    if disk_period is None:
        if hasattr(system, 'Pprimary'):
            ...
            disk_period = (2.0 * scipy.pi * system.st_rad[i]
                           /
                           system.st_vsini[i])
    ...
    if solve:
        if initial_eccentricity == 'solve':
            initial_eccentricity = scipy.optimize.brentq(e_solver_callable,
                                                         system.pl_orbeccen[i],
                                                         0.5,
                                                         xtol=1e-2,
                                                         rtol=1e-2)
        e_solver_callable(initial_eccentricity)
    else:
        e_solver_callable.porb_initial = system.pl_orbper[i]
    
    
    pprint(vars(e_solver_callable.get_found_evolution(initial_eccentricity)))
...
for i in range(0,size):
    ...
    #If the density if not provided it is solved for and converted to the correct units
    density = system.pl_dens[i]
    if numpy.isnan(density) == True:
        calc_dens = (system.pl_massj[i] * constants.M_jup / constants.M_sun) / ((4.0/3.0)*numpy.pi*system.pl_rads[i]**3)
        density = (density * 1.989e33) / (3.367e32)
        
    #Depending on the density of the planet, the logQ value is set accordingly
    if density < 2.0:
        lgQ = 6.0
    else:
        lgQ = 2.0

    #a disctionary containing the dissipation arguments for configuring the 
    #dissipation for the star and the planet
    diss = {'primary': {'tidal_frequency_breaks': None,
                               'spin_frequency_breaks': None,
                               'tidal_frequency_powers': numpy.array([0.0]),
                               'spin_frequency_powers': numpy.array([0.0]),
                               'reference_phase_lag': 0.0*phase_lag(7.0)},
            'secondary': {'tidal_frequency_breaks': None,
                               'spin_frequency_breaks': None,
                               'tidal_frequency_powers': numpy.array([0.0]),
                               'spin_frequency_powers': numpy.array([0.0]),
                               'reference_phase_lag': phase_lag(lgQ)}}
    
    evolution = find_evolution(system, stellar_params, interp, diss, initial_eccentricity='solve')

...
class PriorTransform:
    ...
    def __call__(self, u):
        ...
        if min(iconv(numpy.linspace(iconv.min_age, stellar_age, 1000))) <= 0:
            ...
            return None
        if self.logger is not None: self.logger.debug("SS: means %(x)s" % dict(x=repr(self.means)))
        if self.logger is not None: self.logger.debug("SS: stdev %(x)s" % dict(x=repr(self.standard_deviations)))
        secondary_radius_is_found = False
        a = 'transit depth' in self.means
        if a:
            if self.logger is not None: self.logger.debug("XX: Transit depth is found in self.means.")
            ...
            if aa:
               if self.logger is not None: self.logger.debug("XY: Transit depth uncertainties are found in self.stdevs")
               ...
               if aaa:
                   if self.logger is not None: self.logger.debug("XZ: transit depth uncertainties are not nan")
                   ...
                   if self.logger is not None: self.logger.debug("XZ: transit depth = %(x)f" % dict(x=transit_depth))
                   secondary_radius = (transit_depth**0.5)*primary_radius * const.R_sun.value/const.R_earth.value
                   if self.logger is not None: self.logger.debug("XZ: secondary radius = %(x)f, for transit depth = %(y)f" % dict(x=secondary_radius, y=transit_depth))
                   if secondary_radius is not None:
                       if not math.isnan(secondary_radius):
                           secondary_radius_is_found = True
        b = 'ratio of planet to stellar radius' in self.means
        if b and (not secondary_radius_is_found):
            if self.logger is not None: self.logger.debug("AA: ratio of Rp and Rs is found in self.means.")
            ...
            if bb:
                if self.logger is not None: self.logger.debug("AB: ratio uncertainties are found in self.stdevs")
                ...
                if bbb:
                    if self.logger is not None: self.logger.debug("AC: ratio uncertainties are not nan")
                    ...
                    if self.logger is not None: self.logger.debug("AC: ratio = %(x)f" % dict(x=ratio_of_planet_to_stellar_radius))
                    secondary_radius = ratio_of_planet_to_stellar_radius * primary_radius * const.R_sun.value/const.R_earth.value
                    if self.logger is not None: self.logger.debug("AC: secondary radius = %(x)f" % dict(x=secondary_radius))
                    if secondary_radius is not None:
                        if not math.isnan(secondary_radius):
                            secondary_radius_is_found = True
        c = 'secondary radius' in self.means
        if c and (not secondary_radius_is_found):
            if self.logger is not None: self.logger.debug("PP: secondary radius is found in self.means.")
            ...
            if cc:
                if self.logger is not None: self.logger.debug("PQ: secondary radius uncertainties are found in self.stdevs")
                ...
                if ccc:
                    if self.logger is not None: self.logger.debug("PR: secondary radius uncertainties are not nan")
                    ...
                    if self.logger is not None: self.logger.debug("PR: secondary radius = %(x)f" % dict(x=secondary_radius))
                    if secondary_radius is not None:
                        if not math.isnan(secondary_radius):
                            secondary_radius_is_found = True
        if not secondary_radius_is_found:

...
class EccentricityDistribution(SuperEccentricityDistribution):
    def distribution_of_present_eccentricity(self, e_now):
        if self.e_now_stdev == 0: return 0
        return math.exp(-0.5 * ((e_now - self.mean_e_now) / self.e_now_stdev) ** 2) / self.e_now_stdev / math.sqrt(2 * math.pi)
    def distribution_of_envelop_eccentricity(self, e_env):
        if self.e_env_stdev == 0: return 0
        return math.exp(-0.5 * ((e_env - self.mean_e_env) / self.e_env_stdev) ** 2) / self.e_env_stdev / math.sqrt(2 * math.pi)
    ...
    def integrand_of_the_distribution_of_eccentricity_integral(self, e_now, e):
        return self.integral_of_the_distribution_of_envelop_eccentricity(e, e_now)[0] * self.distribution_of_present_eccentricity(e_now)
    def distribution_of_eccentricity_by_quad(self, e):
        if e>1 or e<0:
            return 0
        return quad(self.integrand_of_the_distribution_of_eccentricity_integral, 0, e, args =(e))
    def distribution_of_eccentricity_by_nquad(self, e):
        if e>1 or e<0:
            return 0
        return nquad(lambda e_now, e_envelop: self.distribution_of_present_eccentricity(e_now) * self.distribution_of_envelop_eccentricity(e_envelop)/(e_envelop-e_now), [self.bounds_e_now(e), self.bounds_e_env(e)])
...
#class SuperBayesianAnalysis(metaclass = ABCMeta):
#    def __init__(self):
#        @abstractmethod
#        def probability_of_x_given_y(self):
#            pass
...
class BayesianAnalysis:
    ...
    def pick_a_tuple_from_the_multi_variable_Gaussian_distribution_old(self, mean, standard_deviation):
        y = random.uniform(0,1)
        r = math.sqrt(-2 * math.log(y*self.product(standard_deviation)*math.pow(2*math.pi,len(mean)/2),math.e))
        sample = []
    ...
    def pick_a_tuple_from_the_multi_variable_Gaussian_distribution(self, mean, standard_deviation):
        n_cross_n_dimensional_array_of_standard_deviations = np.diag(np.array([element **2 for element in standard_deviation]))
        return np.random.default_rng().multivariate_normal(numpy.array(mean),
                                                           n_cross_n_dimensional_array_of_standard_deviations )
    ...
    def workout_envelop_eccentricity(self, nsegments = 10):
        ...
        for i in range(0, len(self.orbital_period)):
            if not (math.isnan(self.orbital_period[i])
                ...
                if self.orbital_period[i] <= 10 and (self.primary_mass[i] > 0.4 and self.primary_mass[i] < 1.2) and (self.metallicity[i] > -1.014 and self.metallicity[i] < 0.537):
    ...
    def construct_probability_density_distribution_of_eccentricity(self,
        ...
        if mean_e_env == -1:
            ...
            w =lambda e_now: b.distribution_of_present_eccentricity(e_now)/(1-e_now)
            def f(e):
                value = quad(w, 0,e)
                return value
            return f
        ...
        if  mean_e_env ==0:
            print('Envelop eccentricity is zero')
            def f(e):
                return 0
            return f

        print('Mean envelop eccentricity is ', mean_e_env)
        def f(e):
            return b.distribution_of_eccentricity_by_nquad(e)

        return f


    def log_prob(self, theta, probability_density_distribution_of_eccentricity):
        ...
        print(repr(dir(b)))
        if not (eccentricity_now == 0 or eccentricity_now<0): return np.log(probability_density_distribution_of_eccentricity(eccentricity_now)[0])
    ...
    def create_initial_theta_and_probability_density_distribution_of_eccentricity(self, j):
        ...
        if j>len(self.planet_name)-1 or j<0: return
        ...
        for i in range(0, len(self.planet_name)):
            if not (math.isnan(self.orbital_period[i])
                ...
                if self.orbital_period[i] <= 10 and (self.primary_mass[i] > 0.4 and self.primary_mass[i] < 1.2) and (
                    ...
                    k = k+1
                    if k == j:
                        theta0 = np.array([self.primary_mass[i],
                        ...
                        f = self.construct_probability_density_distribution_of_eccentricity(mean_e_now=self.eccentricity_now[i],
                                                                                            e_now_stdev=(-self.eccentricity_now_lower_uncertainty[i]+self.eccentricity_now_upper_uncertainty[i])/2,
                                                                                            orbital_period=self.orbital_period[i])
                        return (theta0, f)
        return (None, None)

    def probability_density_distribution_of_eccentricity(self):
        return

    def MCMC(self):
        for j in range(0, len(self.orbital_period)-1):
            theta0, f = self.create_initial_theta_and_probability_density_distribution_of_eccentricity(j)
     #       if not (math.isnan(theta0) or math.isnan(f)):
        ...
        theta0, f = self.create_initial_theta_and_probability_density_distribution_of_eccentricity(j=20)#I am choosing j=20

        print('sss ')

        s = self.log_prob(theta0, f(0.2))

        print('sss ', s)

        sampler = emcee.EnsembleSampler(1, 8, self.log_prob, args=[f[0]])

        state = sampler.run_mcmc(theta0, 100)
        sampler.reset()
